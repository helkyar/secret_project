...
def main(def_args=sys.argv[1:]):
    ...
    if days_before_end < 0:
        sys.exit('days_after must not be negative')
    if days_before_end > 0 and days_after > 0:
        sys.exit('days_after and days_before_end are incompatible')
    days_modifier = days_after - days_before_end
    print(days_modifier)
    os.mkdir(directory)
    os.chdir(directory)
    run(['git', 'init', '-b', 'main'])
    ...
    for day in (start_date + timedelta(n) for n
            in range(days_before + days_modifier)):
...
def arguments(argsval):
    ...
    parser.add_argument('-dbe', '--days_before_end', type=int, default=0,
                        required=False, help="""Specifies the number of days
                        before the current date until which the script will be
                        adding commits. For example: if it is set to 30 the
                        last commit will be on a previous date which is the
